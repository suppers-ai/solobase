...
test.describe('Blog: Admin', () => {
  ...
  test('GET /admin/users accessible with token', async ({ request }) => {
    const { token } = await signup(request);
    const res = await request.get('/admin/users', {
      headers: { Authorization: `Bearer ${token}` },
    });
  ...
  test('GET /admin/database/collections accessible', async ({ request }) => {
    const { token } = await signup(request);
    const res = await request.get('/admin/database/collections', {
      headers: { Authorization: `Bearer ${token}` },
    });
    expect([200, 403]).toContain(res.status());
  });
