<script lang="ts">
	...
	import { auth, currentUser, userRoles } from '$lib/stores/auth';
	...
	// Only run this check after auth has been checked (not during initial load)
	$: if (authChecked && typeof window !== 'undefined') {
		...
		// If no user at all, onMount will handle the redirect to login
		if (user && isAdminPage) {
			const isAdmin = roles && roles.includes('admin');
			if (!isAdmin) {
				console.log('Non-admin user attempting to access admin page, redirecting...', { roles, isAdmin });
				goto('/profile');
			}
		}
	}
