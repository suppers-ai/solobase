...
use crate::helpers::*;
use crate::{PRICING_COLLECTION, PRODUCTS_COLLECTION};

pub fn handle_calculate(msg: &Message) -> BlockResult {
    ...
    let body: CalcReq = match decode_body(msg) {
        Ok(b) => b,
        Err(r) => return r,
    };
    ...
    let template_id = str_field(&product, "pricing_template_id");
    ...
    let formula = str_field(&template, "price_formula");
    if formula.is_empty() {
