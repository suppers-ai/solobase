...
/// Parse pagination query params from message meta.
...
pub fn respond_binary(msg: &Message, data: Vec<u8>, content_type: &str, extra_headers: &[(&str, &str)]) -> BlockResult {
    let mut meta = vec![MetaEntry {
        key: "resp.content_type".to_string(),
        value: content_type.to_string(),
    }];
    for (k, v) in extra_headers {
        meta.push(MetaEntry { key: k.to_string(), value: v.to_string() });
    }
    BlockResult {
        action: Action::Respond,
        response: Some(Response { data, meta }),
        error: None,
        message: Some(msg.clone()),
    }
}
