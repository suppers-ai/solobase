...
# Create a temporary environment to populate the database
export SOLOBASE_DB_PATH="$SCRIPT_DIR/demo.db"
export SUPPERS_AI__AUTH__ADMIN_EMAIL=admin@example.com
export SUPPERS_AI__AUTH__ADMIN_PASSWORD=admin123
export SUPPERS_AI__AUTH__JWT_SECRET=$(openssl rand -hex 32)

echo "JWT Secret set"

# Build and run solobase to create the database
echo "Building solobase..."
(cd "$SOLOBASE_DIR/crates/solobase" && cargo build --release -q)
