...
echo "--------------------------"
# Set JWT secret if not already set (check if it exists first)
if ! fly secrets list --app $APP_NAME 2>/dev/null | grep -q "SUPPERS_AI__AUTH__JWT_SECRET"; then
    echo "Setting SUPPERS_AI__AUTH__JWT_SECRET..."
    fly secrets set SUPPERS_AI__AUTH__JWT_SECRET="$(openssl rand -hex 32)" --app $APP_NAME
else
    echo "SUPPERS_AI__AUTH__JWT_SECRET already set"
fi
