	...
	case FieldTypeDate, FieldTypeDateTime:
		...
		case string:
			...
			if field.Type == FieldTypeDate {
				_, err = apptime.ParseWithLayout("2006-01-02", val)
			} else {
				_, err = apptime.ParseWithLayout(apptime.TimeFormat, val)
			}
			if err != nil {
				return &ValidationError{
					Field:   field.Name,
					Type:    "type",
					Message: fmt.Sprintf("invalid date format: %s", val),
				}
			}
		default:
			return &ValidationError{
...
func (v *Validator) validateRules(field *Field, value interface{}) error {
	...
	case FieldTypeText, FieldTypeEmail, FieldTypeURL:
		...
		// Pattern validation is skipped in WASM builds (no regexp support)
		// If a pattern is specified, we skip validation rather than fail
		// This is a graceful degradation for TinyGo compatibility
	...
	case FieldTypeDate, FieldTypeDateTime:
		...
		case string:
			var err error
			if field.Type == FieldTypeDate {
				t, err = apptime.ParseWithLayout("2006-01-02", val)
			} else {
				t, err = apptime.ParseWithLayout(apptime.TimeFormat, val)
			}
			if err != nil {
				return &ValidationError{
					Field:   field.Name,
