...
/// - `{name}_vectors` — id PK, vector BLOB, metadata JSON, [text TEXT]
/// - `{name}_fts` — fts5(id UNINDEXED, text) — only when keyword_search=true
/// - `{name}_meta` — id PK, rowid INTEGER, metadata JSON, [text TEXT]
pub fn build_create_index_sql(prefixed_name: &str, keyword_search: bool) -> Vec<String> {
    ...
    let text_col_vectors = if keyword_search { ", text TEXT" } else { "" };
    let text_col_meta = if keyword_search { ", text TEXT" } else { "" };

    let mut out = vec![
        format!(
            r#"CREATE TABLE "{v}" (id TEXT PRIMARY KEY, vector BLOB NOT NULL, metadata TEXT{text_col_vectors})"#
        ),
    ];
    if keyword_search {
    ...
    out.push(format!(
        r#"CREATE TABLE "{m}" (id TEXT PRIMARY KEY, rowid INTEGER, metadata TEXT{text_col_meta})"#
    ));
    out
}
