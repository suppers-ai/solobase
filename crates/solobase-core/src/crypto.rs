...
mod tests {
    ...
    /// then bearer-wrap it. Used by the auth-ui block-id tests below.
    fn sign_as_block(
        master_secret: &str,
        block_id: &str,
        claims: HashMap<String, serde_json::Value>,
    ...
    #[tokio::test]
    async fn extract_auth_meta_accepts_auth_ui_signed_token() {
        ...
        extract_auth_meta(&ctx, &header, master, "", &mut msg).await;
    ...
    #[tokio::test]
    async fn extract_auth_meta_falls_back_to_master_secret() {
    ...
    #[tokio::test]
    async fn extract_auth_meta_silent_on_garbage_token() {
        let ctx = crate::test_support::TestContext::with_auth().await;
        let mut msg = wafer_run::types::Message::new("http.request");
        extract_auth_meta(&ctx, "Bearer not.a.jwt", "master-secret", "", &mut msg).await;
        assert_eq!(msg.get_meta(wafer_run::meta::META_AUTH_USER_ID), "");
    }
}
