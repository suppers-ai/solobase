...
use crate::blocks::helpers::{hex_encode, sha256_hex, RecordExt};

impl AuthBlock {
    pub(super) async fn handle_api_keys_list(&self, ctx: &dyn Context, msg: &mut Message) -> Result_ {
    ...
    pub(super) async fn handle_api_keys_create(&self, ctx: &dyn Context, msg: &mut Message) -> Result_ {
        let user_id = msg.user_id();
    ...
    pub(super) async fn handle_api_keys_revoke(&self, ctx: &dyn Context, msg: &mut Message) -> Result_ {
        let path = msg.path();
        ...
        let data = crate::blocks::helpers::json_map(serde_json::json!({"revoked_at": crate::blocks::helpers::now_rfc3339()}));
