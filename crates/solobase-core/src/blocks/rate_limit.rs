...
impl UserRateLimiter {
    ...
    pub async fn check(&self, ctx: &dyn Context, key: &str, limit: RateLimit) -> Result<u32, u64> {
        ...
        // reset count + window_start if expired. The dialect-portable SQL
        // (CASE WHEN + CURRENT_TIMESTAMP) is generated by wafer-sql-utils.
        use crate::blocks::auth::RATE_LIMITS_TABLE as RATE_LIMITS;
        let id = super::helpers::sha256_hex(format!("rl:{key}:{now}").as_bytes());
        let (sql, vals) = upsert::build_rate_limit_upsert(
            RATE_LIMITS,
            &id,
            key,
            now,
