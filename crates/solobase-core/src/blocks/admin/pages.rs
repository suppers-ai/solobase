...
async fn users_table(records: &[db::Record], ctx: &dyn Context) -> Markup {
    ...
    let content = html! {
        ...
        div #settings-content {
            @match &settings {
                ...
                Err(e) => {
                    div .login-error { "Failed to load settings: " (e.message) }
                }
            }
        }
    };
                    ...
                    table .table {
                        ...
                        tbody {
                            @if list.records.is_empty() {
                                tr {
                                    td colspan="5" .text-center .text-muted style="padding: 2rem;" { "No API keys" }
                                }
                            }
                            @for record in &list.records {
...
pub async fn logs_page(ctx: &dyn Context, msg: &mut Message) -> Result_ {
    ...
    let content = html! {
        (components::page_header(
            "Audit Logs",
            Some("Track all system activity"),
            Some(html! {
        ...
        div .filter-bar {
            (components::search_input("search", "Search by resource...", "/b/admin/logs", "#logs-content"))
        }
        ...
        div #logs-content {
            @match &result {
                Ok(list) => {
                    div .table-container {
                        table .table {
                            ...
                            tbody {
                                @if list.records.is_empty() {
                                    tr {
                                        td colspan="5" .text-center .text-muted style="padding: 2rem;" { "No logs found" }
                                    }
                                }
                                @for record in &list.records {
                    ...
                    @let total_pages = ((list.total_count as f64) / (list.page_size.max(1) as f64)).ceil() as u32;
                    (components::pagination(list.page as u32, total_pages, "/b/admin/logs", "#logs-content"))
                }
                Err(e) => {
                    div .login-error { "Failed to load logs: " (e.message) }
                }
            }
        }
    };
...
pub async fn handle_create_role(ctx: &dyn Context, msg: &mut Message) -> Result_ {
    let body: std::collections::HashMap<String, serde_json::Value> = match msg.decode() {
        Ok(b) => b,
        Err(e) => return wafer_run::helpers::err_bad_request(msg, &format!("Invalid body: {e}")),
    };
