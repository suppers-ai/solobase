...
impl MockContext {
    ...
    pub fn seed(&self, collection: &str, id: &str, data: HashMap<String, serde_json::Value>) {
        ...
        records.push(Record { id: id.to_string(), data });
    }
    ...
    fn handle_db_call(&self, kind: &str, data: &[u8]) -> Result<Vec<u8>, WaferError> {
        match kind {
            ...
            _ => Err(WaferError::new("not_implemented", format!("unhandled db op: {kind}"))),
        }
    }
    ...
    fn handle_config_call(&self, _kind: &str, data: &[u8]) -> Result<Vec<u8>, WaferError> {
        ...
        match self.config.get(&req.key) {
            ...
            None => Err(WaferError::new("not_found", format!("config key '{}' not found", req.key))),
        }
    }
    ...
    fn db_get(&self, data: &[u8]) -> Result<Vec<u8>, WaferError> {
        #[derive(serde::Deserialize)]
        struct Req { collection: String, id: String }
        let req: Req = serde_json::from_slice(data)
            .map_err(|e| WaferError::new("internal", e.to_string()))?;
        let db = self.db.lock().unwrap();
        let records = db.get(&req.collection).ok_or_else(|| WaferError::new("not_found", "not found"))?;
        let record = records.iter().find(|r| r.id == req.id)
    ...
    fn db_list(&self, data: &[u8]) -> Result<Vec<u8>, WaferError> {
        ...
        #[derive(serde::Deserialize)]
        struct SortDef { field: String, #[serde(default)] desc: bool }
        ...
        let mut filtered: Vec<&Record> = records.iter()
            .filter(|r| {
                req.filters.iter().all(|f| matches_filter(r, f))
            })
            .collect();
        ...
        for s in req.sort.iter().rev() {
            filtered.sort_by(|a, b| {
                ...
                if s.desc { cmp.reverse() } else { cmp }
            });
        }
        ...
        let page_records: Vec<Record> = filtered.into_iter()
            .skip(offset)
            .take(limit)
            .cloned()
            .collect();

        let page = if req.limit > 0 { req.offset / req.limit + 1 } else { 1 };
    ...
    fn db_create(&self, data: &[u8]) -> Result<Vec<u8>, WaferError> {
        #[derive(serde::Deserialize)]
        struct Req { collection: String, data: HashMap<String, serde_json::Value> }
        let req: Req = serde_json::from_slice(data)
            .map_err(|e| WaferError::new("internal", e.to_string()))?;
    ...
    fn db_update(&self, data: &[u8]) -> Result<Vec<u8>, WaferError> {
        #[derive(serde::Deserialize)]
        struct Req { collection: String, id: String, data: HashMap<String, serde_json::Value> }
        let req: Req = serde_json::from_slice(data)
            .map_err(|e| WaferError::new("internal", e.to_string()))?;

        let mut db = self.db.lock().unwrap();
        let records = db.get_mut(&req.collection)
            .ok_or_else(|| WaferError::new("not_found", "not found"))?;
        let record = records.iter_mut().find(|r| r.id == req.id)
            .ok_or_else(|| WaferError::new("not_found", "not found"))?;
        for (k, v) in req.data {
    ...
    fn db_delete(&self, data: &[u8]) -> Result<Vec<u8>, WaferError> {
        ...
        let records = db.get_mut(&req.collection)
            .ok_or_else(|| WaferError::new("not_found", "not found"))?;
        let idx = records.iter().position(|r| r.id == req.id)
            .ok_or_else(|| WaferError::new("not_found", "not found"))?;
        records.remove(idx);
        Ok(b"{}".to_vec())
    }
    ...
    fn db_count(&self, data: &[u8]) -> Result<Vec<u8>, WaferError> {
        #[derive(serde::Deserialize)]
        struct Req { collection: String, #[serde(default)] filters: Vec<FilterDef> }
...
fn matches_filter(record: &Record, filter: &FilterDef) -> bool {
    ...
    match filter.operator.as_str() {
        ...
        "gte" => matches!(compare_values(val, &filter.value), std::cmp::Ordering::Greater | std::cmp::Ordering::Equal),
        "lt" => compare_values(val, &filter.value) == std::cmp::Ordering::Less,
        "lte" => matches!(compare_values(val, &filter.value), std::cmp::Ordering::Less | std::cmp::Ordering::Equal),
        _ => true,
    }
}
