  ...
  await page.addInitScript(() => {
    ...
    window.fetch = function(input: RequestInfo | URL, init?: RequestInit) {
      ...
      if (url.startsWith('/api/')) {
        ...
        else if (input instanceof URL) input = new URL(url, window.location.origin);
...
test.describe('New Developer Click-Through Journey', () => {
  ...
  async function goToDashboard(page: Page, hash = 'overview') {
    ...
    await page.context().addCookies([{
      name: 'auth_token', value: devToken,
      domain: '127.0.0.1', path: '/',
      httpOnly: true, sameSite: 'Lax',
    }]);
    await page.goto(`/blocks/dashboard/frontend/#${hash}`);
    await expect(page.locator('header').filter({ hasText: 'Solobase' })).toBeVisible({ timeout: 15000 });
  }
  ...
  test('browse the landing page like a new visitor', async ({ page }) => {
    ...
    // Nav has key links
    await expect(page.locator('header nav a', { hasText: 'Docs' })).toBeVisible();
    await expect(page.locator('header nav a', { hasText: 'GitHub' })).toBeVisible();
    ...
    expect(blockTitles.map(t => t.trim())).toEqual([
      ...
      'Products & Payments', 'Admin Panel', 'User Dashboard',
    ]);
  ...
  test('click Docs link from landing page and browse documentation', async ({ page }) => {
    await page.goto('/');
    ...
    await expect(sidebar.locator('h4', { hasText: 'Getting Started' })).toBeVisible();
    await expect(sidebar.locator('h4', { hasText: 'Authentication' })).toBeVisible();
    await expect(sidebar.locator('h4', { hasText: 'WAFER Runtime' })).toBeVisible();
  ...
  test('click Sign Up from the landing page and create an account', async ({ page }) => {
    ...
    await page.goto('/blocks/dashboard/frontend/');
    ...
    await page.goto('/blocks/dashboard/frontend/#overview');
  ...
  test('click through Overview tab — see stats and get started section', async ({ page }) => {
  ...
  test('click Deployments tab, create a deployment, then delete it', async ({ page }) => {
    await goToDashboard(page);

    // Click Deployments tab
    await page.getByRole('button', { name: 'Deployments' }).click();
    await expect(page.getByRole('heading', { name: 'Deployments', exact: true })).toBeVisible({ timeout: 10000 });
    ...
    await expect(page.getByText('Deployment created successfully')).toBeVisible({ timeout: 5000 });
    ...
    await expect(page.getByText('Deployment deleted')).toBeVisible({ timeout: 5000 });

    // Deployment should be gone — either empty state or no pending badge
    await expect(
      page.getByText('No deployments yet')
        .or(page.getByRole('button', { name: 'Create Deployment' }))
    ).toBeVisible({ timeout: 5000 });
  });

  test('click API Keys tab, create a key, see the key, then revoke it', async ({ page }) => {
    await goToDashboard(page);

    // Click API Keys tab
    await page.getByRole('button', { name: 'API Keys' }).click();
    await expect(page.getByText('API Keys').first()).toBeVisible({ timeout: 10000 });
    ...
    await expect(page.locator('code').filter({ hasText: 'sb_' })).toBeVisible({ timeout: 10000 });
    await expect(page.getByRole('button', { name: 'Copy' })).toBeVisible();

    // Dismiss the key banner
    await page.getByRole('button', { name: 'Dismiss' }).click();
    await expect(page.locator('code').filter({ hasText: 'sb_' })).not.toBeVisible();
    ...
    const [revokeResponse] = await Promise.all([
      page.waitForResponse(
        resp => resp.url().includes('/auth/api-keys') && resp.request().method() === 'DELETE',
        { timeout: 10000 }
      ),
    ...
    await expect(page.getByText('API key revoked')).toBeVisible({ timeout: 5000 });
  });

  test('click Settings tab, update display name, verify it saves', async ({ page }) => {
    await goToDashboard(page);
    ...
    await expect(page.getByText('Welcome back, Test Developer')).toBeVisible({ timeout: 10000 });
  ...
  test('create resources and see Overview stats update', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Deployments', exact: true })).toBeVisible({ timeout: 10000 });
    await page.getByRole('button', { name: 'Create Deployment' }).first().click();
    await page.locator('input[placeholder="my-backend"]').fill('stats-test-app');
    await page.locator('button[type="submit"]', { hasText: 'Create' }).click();
    await expect(page.getByText('Deployment created successfully')).toBeVisible({ timeout: 5000 });
    ...
    await expect(main.locator('text=/^[1-9]/').first()).toBeVisible({ timeout: 10000 });
  ...
  test('click Logout, then log back in through the login form', async ({ page }) => {
    await goToDashboard(page);
    ...
    await expect(page.getByText('Welcome back, Test Developer')).toBeVisible({ timeout: 15000 });
  });
});
