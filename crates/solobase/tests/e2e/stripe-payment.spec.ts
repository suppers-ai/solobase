...
test.describe("Stripe Payment Flow", () => {
  ...
  test("admin creates a product group", async ({ request }) => {
    const res = await request.post("/b/products/api/admin/groups", {
  ...
  test("admin creates a paid product", async ({ request }) => {
    const res = await request.post("/b/products/api/admin/products", {
      headers: { Authorization: `Bearer ${adminToken}` },
      data: {
        name: "Pro Plan",
  ...
  test("Stripe webhook completes the purchase", async ({ request }) => {
    const result = await simulateCheckoutComplete(
      baseUrl,
      checkoutSessionId,
      purchaseId,
    );
  ...
  test("webhook rejects expired timestamp (replay protection)", async ({
    request,
  }) => {
    const event = {
      id: "evt_test_old",
  ...
  test("Stripe webhook handles refund", async ({ request }) => {
    ...
    const purchaseData =
      purchaseBody.purchase?.data || purchaseBody.data || purchaseBody;
