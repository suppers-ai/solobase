...
test.describe("Browser Checkout Flow", () => {
  ...
  test("setup: get admin token", async ({ request }) => {
    ...
    let res = await request.post("/b/auth/api/login", {
      data: { email, password },
    });
    if (!res.ok()) {
      res = await request.post("/b/auth/api/signup", {
  ...
  test("setup: create user account", async ({ request }) => {
    const body = await apiPost(request, "/b/auth/api/signup", {
      email: userEmail,
  ...
  test("user sees product in catalog", async ({ request }) => {
    ...
    const found = records.find(
      (r: any) => (r.id || r.data?.id)?.toString() === productId,
    );
  ...
  test("browser: navigate to checkout and click Pay Now", async ({ page }) => {
    ...
    await page.waitForURL(
      (url) => url.searchParams.get("checkout") === "success",
      { timeout: 10_000 },
    );
  ...
  test("purchase is marked as completed after browser checkout", async ({
    request,
  }) => {
    ...
    const body = await apiGet(
      request,
      `/b/products/purchases/${purchaseId}`,
      userToken,
    );
  ...
  test("admin sees the completed purchase", async ({ request }) => {
    const body = await apiGet(
      request,
      `/b/products/api/admin/purchases/${purchaseId}`,
      adminToken,
    );
    const data = body.purchase?.data || body.data || body;
    expect(data.status).toBe("completed");
  ...
  test("multi-item purchase is completed", async ({ request }) => {
    ...
    const body = await apiGet(
      request,
      `/b/products/purchases/${multiPurchaseId}`,
      userToken,
    );
    const data = body.purchase?.data || body.data || body;
    expect(data.status).toBe("completed");
  ...
  test("cancelled purchase remains pending", async ({ request }) => {
    const body = await apiGet(
      request,
      `/b/products/purchases/${cancelPurchaseId}`,
      userToken,
    );
    const data = body.purchase?.data || body.data || body;
