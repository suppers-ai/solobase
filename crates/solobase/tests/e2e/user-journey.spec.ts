...
test("setup: create admin user", async ({ request }) => {
  ...
  if (!roles.includes("admin")) {
    console.log(
      "Warning: admin-journey@test.com does not have admin role. Admin tests will skip.",
    );
  }
});
...
test.describe("Plans & Pricing", () => {
  test("admin can create product plans", async ({ request }) => {
    ...
    expect(
      groupRes.ok(),
      `Group: ${groupRes.status()} ${await groupRes.text()}`,
    ).toBeTruthy();
    ...
    expect(
      createRes.ok(),
      `Product: ${createRes.status()} ${await createRes.text()}`,
    ).toBeTruthy();
...
test.describe("Deployments", () => {
  test("developer can create a deployment", async ({ request }) => {
    ...
    expect(["pending", "active", "failed"]).toContain(
      deployment.data?.status || deployment.status,
    );
  });
...
test.describe("Full User Journey", () => {
  test("end-to-end: signup, browse plans, create deployment, manage, delete", async ({
    request,
  }) => {
    ...
    const detailRes = await request.get(`/b/deployments/${deployId}`, {
      headers: h,
    });
    ...
    const deleteRes = await request.delete(`/b/deployments/${deployId}`, {
      headers: h,
    });
    expect(deleteRes.ok()).toBeTruthy();
    ...
    const afterDeleteRes = await request.get(`/b/deployments/${deployId}`, {
      headers: h,
    });
...
test.describe("Dashboard Frontend", () => {
  test("dashboard HTML page loads", async ({ request }) => {
    const res = await request.get("/b/admin/");
    expect(res.ok()).toBeTruthy();
