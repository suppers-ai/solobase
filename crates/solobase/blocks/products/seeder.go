...
type Seeder interface {
	...
	SeedVariables(db database.Service) ([]models.Variable, error)

	// SeedGroupTemplates seeds custom group templates
	SeedGroupTemplates(db database.Service) ([]models.GroupTemplate, error)

	// SeedProductTemplates seeds custom product templates
	SeedProductTemplates(db database.Service) ([]models.ProductTemplate, error)

	// SeedPricingTemplates seeds custom pricing templates
	SeedPricingTemplates(db database.Service) ([]models.PricingTemplate, error)

	// ShouldSeed returns whether seeding should occur
	// Can be used to check if data already exists
	ShouldSeed(db database.Service) bool
}
...
func (d *DefaultSeeder) ShouldSeed(db database.Service) bool {
	count, _ := db.Count(context.Background(), "ext_products_variables", nil)
	return count == 0 // Only seed if no data exists
}

// SeedVariables returns the default variables
func (d *DefaultSeeder) SeedVariables(db database.Service) ([]models.Variable, error) {
	return DefaultVariables(), nil
}

// SeedGroupTemplates returns the default group templates
func (d *DefaultSeeder) SeedGroupTemplates(db database.Service) ([]models.GroupTemplate, error) {
	return DefaultGroupTemplates(), nil
}

// SeedProductTemplates returns the default product templates
func (d *DefaultSeeder) SeedProductTemplates(db database.Service) ([]models.ProductTemplate, error) {
	return DefaultProductTemplates(), nil
}

// SeedPricingTemplates returns the default pricing templates
func (d *DefaultSeeder) SeedPricingTemplates(db database.Service) ([]models.PricingTemplate, error) {
...
type CustomSeeder struct {
	...
	CustomVariables        func(db database.Service) ([]models.Variable, error)
	CustomGroupTemplates   func(db database.Service) ([]models.GroupTemplate, error)
	CustomProductTemplates func(db database.Service) ([]models.ProductTemplate, error)
	CustomPricingTemplates func(db database.Service) ([]models.PricingTemplate, error)
	CustomShouldSeed       func(db database.Service) bool
}

// SeedVariables returns custom or default variables
func (c *CustomSeeder) SeedVariables(db database.Service) ([]models.Variable, error) {
...
func (c *CustomSeeder) SeedGroupTemplates(db database.Service) ([]models.GroupTemplate, error) {
...
func (c *CustomSeeder) SeedProductTemplates(db database.Service) ([]models.ProductTemplate, error) {
...
func (c *CustomSeeder) SeedPricingTemplates(db database.Service) ([]models.PricingTemplate, error) {
...
// ShouldSeed checks if seeding should occur
func (c *CustomSeeder) ShouldSeed(db database.Service) bool {
...
func SeedWithSeeder(db database.Service, seeder Seeder) error {
	...
	for _, v := range variables {
		...
		_, err := db.ExecRaw(ctx, `INSERT INTO ext_products_variables (name, display_name, value_type, type, default_value, description, status, created_at, updated_at)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
	...
	for _, gt := range groupTemplates {
		...
		_, err := db.ExecRaw(ctx, `INSERT INTO ext_products_group_templates (name, display_name, description, icon, filter_fields_schema, status, created_at, updated_at)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
	...
	for _, pt := range productTemplates {
		...
		_, err := db.ExecRaw(ctx, `INSERT INTO ext_products_product_templates
	...
	for _, pt := range pricingTemplates {
		...
		_, err := db.ExecRaw(ctx, `INSERT INTO ext_products_pricing_templates
