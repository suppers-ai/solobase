...
func (c *Client) CreateCheckoutSession(params *CheckoutSessionParams) (*CheckoutSession, error) {
	...
	// Add Stripe Connect application fee if specified
	if params.ApplicationFeeAmount > 0 {
		data.Set("payment_intent_data[application_fee_amount]", strconv.FormatInt(params.ApplicationFeeAmount, 10))
	}

	// Use connected account if specified
	resp, err := c.requestWithAccount("POST", "/checkout/sessions", data, params.StripeAccount)
	if err != nil {
		return nil, err
	}
...
func (c *Client) request(method, path string, data url.Values) ([]byte, error) {
	return c.requestWithAccount(method, path, data, "")
}

// requestWithAccount makes a request to the Stripe API, optionally on behalf of a connected account
func (c *Client) requestWithAccount(method, path string, data url.Values, stripeAccount string) ([]byte, error) {
	var body []byte
	if data != nil {
