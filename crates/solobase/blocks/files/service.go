...
import (
	...
	"github.com/suppers-ai/solobase/core/fileutil"
	"github.com/suppers-ai/solobase/core/uuid"
)

// validStorageNameRegex allows alphanumeric, spaces, dashes, underscores, dots, and common international characters
var validStorageNameRegex = regexp.MustCompile(`^[\p{L}\p{N}\s._-]+$`)

// validateStorageName validates a storage name (bucket, folder, or file name) for security
// Returns an error if the name contains path traversal patterns, null bytes, or invalid characters
func validateStorageName(name string, nameType string) error {
	...
	if strings.Contains(name, "\x00") {
		return fmt.Errorf("%s name contains invalid characters", nameType)
	}

	// Check for path traversal patterns
	if strings.Contains(name, "..") {
		return fmt.Errorf("%s name cannot contain path traversal sequences", nameType)
	}

	// Check for absolute path indicators
	if strings.HasPrefix(name, "/") || strings.HasPrefix(name, "\\") {
		return fmt.Errorf("%s name cannot start with a path separator", nameType)
	}

	// Check for path separators within the name
	if strings.Contains(name, "/") || strings.Contains(name, "\\") {
		return fmt.Errorf("%s name cannot contain path separators", nameType)
	}

	// Validate against allowed character pattern
	if !validStorageNameRegex.MatchString(name) {
		return fmt.Errorf("%s name contains invalid characters; only letters, numbers, spaces, dots, dashes, and underscores are allowed", nameType)
	}
...
type StorageService struct {
	...
	provider adapterstorage.Provider
	repo     *storageRepository
	appID    string // Application ID for storage isolation
	ctx      context.Context
}

func NewStorageService(repo *storageRepository, cfg config.StorageConfig) *StorageService {
...
func NewStorageServiceWithOptions(repo *storageRepository, cfg config.StorageConfig, opts *StorageOptions) *StorageService {
	var provider adapterstorage.Provider
	var err error
	...
	if localPath == "" || localPath == "./data/storage" || localPath == constants.DefaultStoragePath || localPath == constants.DefaultStoragePath+"/int" {
		localPath = constants.DefaultStoragePath // Base storage path - buckets will be subdirectories
	}
	...
	// Use storage factory to create provider
	factory := adapterstorage.GetFactory()
	if factory == nil {
		log.Printf("Storage factory not set, using noop provider")
		provider = adapterstorage.NewNoopProvider()
	} else {
		switch cfg.Type {
		case "s3":
			storageConfig := adapterstorage.Config{
				Provider:          adapterstorage.ProviderS3,
			...
			provider, err = factory.NewS3Provider(storageConfig)
			if err != nil {
				log.Printf("Failed to initialize S3 storage: %v, falling back to noop", err)
				provider = adapterstorage.NewNoopProvider()
			}
		default:
			localConfig := adapterstorage.Config{
				Provider: adapterstorage.ProviderLocal,
				BasePath: localPath,
			}
			provider, err = factory.NewLocalProvider(localConfig)
			if err != nil {
...
func (s *StorageService) GetObjectInfo(bucket, objectID string) (*adapterstorage.StorageObject, error) {
	...
	if obj.BucketName != bucket {
		...
		return nil, fmt.Errorf("object %s not found in bucket %s", objectID, bucket)
	}
...
func (s *StorageService) CreateBucket(name string, public bool) error {
	...
	if err := validateStorageName(name, "bucket"); err != nil {
		return err
	}
	...
	err = s.provider.CreateBucket(s.ctx, name, adapterstorage.CreateBucketOptions{Public: public})
	if err != nil {
		if !strings.Contains(err.Error(), "exists") && !strings.Contains(err.Error(), "exist") {
			return err
		}
...
func (s *StorageService) GetBuckets() ([]interface{}, error) {
	...
	// Get buckets with stats in a single query (fixes N+1)
	buckets, bucketStats, err := s.repo.ListBucketsWithStats(s.ctx)
	if err != nil {
		// If repository doesn't support this operation, return empty list
	...
	for i, bucket := range buckets {
		// Get stats from pre-fetched map
		count := int64(0)
		totalSize := int64(0)
		if stats, ok := bucketStats[bucket.Name]; ok {
...
func (s *StorageService) GetObjects(bucket string, userID string, parentFolderID *string) ([]interface{}, error) {
	...
	objects, err := s.repo.ListObjects(s.ctx, ListObjectsOptions{
		BucketName:     bucket,
		UserID:         userIDPtr,
		AppID:          appIDPtr,
...
func (s *StorageService) UploadFile(bucket, filename, userID string, reader io.Reader, size int64, mimeType string, parentFolderID *string) (interface{}, error) {
	...
	if err := validateStorageName(filename, "file"); err != nil {
		return nil, err
	}
	...
	err := s.provider.PutObject(s.ctx, bucket, storageKey, &buf, size, adapterstorage.PutObjectOptions{
...
func (s *StorageService) CreateFolderWithParent(bucket, folderName, userID string, parentFolderID *string) (string, error) {
	...
	if err := validateStorageName(folderName, "folder"); err != nil {
		return "", err
	}
	...
	err = s.provider.PutObject(s.ctx, bucket, keepFilePath, bytes.NewReader([]byte("")), 0, adapterstorage.PutObjectOptions{
...
func (s *StorageService) GetRecentlyViewed(userID string, limit int) ([]*adapterstorage.StorageObject, error) {
...
func (s *StorageService) SearchStorageObjects(userID, appID, query string, limit int) ([]*adapterstorage.StorageObject, error) {
	searchPattern := "%" + query + "%"
...
func (s *StorageService) RenameObject(bucket, objectID, newName string) error {
	// Validate new name for security
	if err := validateStorageName(newName, "object"); err != nil {
		return err
	}
	...
	if obj.ContentType != "application/x-directory" {
		...
		if err := s.provider.PutObject(s.ctx, bucket, newKey, bytes.NewReader(content), int64(len(content)), adapterstorage.PutObjectOptions{
...
// CreateUploadToken creates a new upload token for resumable uploads.
func (s *StorageService) CreateUploadToken(ctx context.Context, token *UploadToken) error {
	if s.repo == nil {
		return fmt.Errorf("repository not available")
	}
	return s.repo.CreateUploadToken(ctx, token)
}

// GetUploadToken retrieves an upload token by ID.
func (s *StorageService) GetUploadToken(ctx context.Context, id string) (*UploadToken, error) {
	if s.repo == nil {
		return nil, ErrNotFound
	}
	return s.repo.GetUploadToken(ctx, id)
}

// GetUploadTokenByToken retrieves an upload token by its token string.
func (s *StorageService) GetUploadTokenByToken(ctx context.Context, token string) (*UploadToken, error) {
	if s.repo == nil {
		return nil, ErrNotFound
	}
	return s.repo.GetUploadTokenByToken(ctx, token)
}

// UpdateUploadTokenProgress updates the bytes uploaded for an upload token.
func (s *StorageService) UpdateUploadTokenProgress(ctx context.Context, id string, bytesUploaded int64) error {
	if s.repo == nil {
		return fmt.Errorf("repository not available")
	}
	return s.repo.UpdateUploadTokenProgress(ctx, id, bytesUploaded)
}

// CompleteUploadToken marks an upload token as completed.
func (s *StorageService) CompleteUploadToken(ctx context.Context, id, objectID string) error {
	if s.repo == nil {
		return fmt.Errorf("repository not available")
	}
	return s.repo.CompleteUploadToken(ctx, id, objectID)
}

// DeleteUploadToken deletes an upload token.
func (s *StorageService) DeleteUploadToken(ctx context.Context, id string) error {
	if s.repo == nil {
		return fmt.Errorf("repository not available")
	}
	return s.repo.DeleteUploadToken(ctx, id)
}

// DeleteExpiredUploadTokens removes all expired upload tokens.
func (s *StorageService) DeleteExpiredUploadTokens(ctx context.Context) error {
	if s.repo == nil {
		return nil
	}
	return s.repo.DeleteExpiredUploadTokens(ctx)
}

// ==================== Download Token Operations ====================

// CreateDownloadToken creates a new download token for direct downloads.
func (s *StorageService) CreateDownloadToken(ctx context.Context, token *DownloadToken) error {
	if s.repo == nil {
		return fmt.Errorf("repository not available")
	}
	return s.repo.CreateDownloadToken(ctx, token)
}

// GetDownloadToken retrieves a download token by ID.
func (s *StorageService) GetDownloadToken(ctx context.Context, id string) (*DownloadToken, error) {
	if s.repo == nil {
		return nil, ErrNotFound
	}
	return s.repo.GetDownloadToken(ctx, id)
}

// GetDownloadTokenByToken retrieves a download token by its token string.
func (s *StorageService) GetDownloadTokenByToken(ctx context.Context, token string) (*DownloadToken, error) {
	if s.repo == nil {
		return nil, ErrNotFound
	}
	return s.repo.GetDownloadTokenByToken(ctx, token)
}

// UpdateDownloadTokenProgress updates the bytes served for a download token.
func (s *StorageService) UpdateDownloadTokenProgress(ctx context.Context, id string, bytesServed int64) error {
	if s.repo == nil {
		return fmt.Errorf("repository not available")
	}
	return s.repo.UpdateDownloadTokenProgress(ctx, id, bytesServed)
}

// CompleteDownloadToken marks a download token as completed.
func (s *StorageService) CompleteDownloadToken(ctx context.Context, id string) error {
	if s.repo == nil {
		return fmt.Errorf("repository not available")
	}
	return s.repo.CompleteDownloadToken(ctx, id)
}

// DeleteDownloadToken deletes a download token.
func (s *StorageService) DeleteDownloadToken(ctx context.Context, id string) error {
	if s.repo == nil {
		return fmt.Errorf("repository not available")
	}
	return s.repo.DeleteDownloadToken(ctx, id)
}

// DeleteExpiredDownloadTokens removes all expired download tokens.
func (s *StorageService) DeleteExpiredDownloadTokens(ctx context.Context) error {
	if s.repo == nil {
		return nil
	}
	return s.repo.DeleteExpiredDownloadTokens(ctx)
}
