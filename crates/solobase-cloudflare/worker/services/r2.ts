...
import type { Message, BlockResult, ProjectConfig } from '../types';
...
function prefixedKey(projectId: string, folder: string, key: string): string {
  if (!validatePathComponent(folder) || !validatePathComponent(key)) {
    throw new Error('invalid path component');
  }
  return `${projectId}/${folder}/${key}`;
}

function folderPrefix(projectId: string, folder: string): string {
  ...
  try {
    switch (msg.kind) {
      ...
      case 'storage.put': {
        ...
        if (db && project && project.plan !== 'platform') {
          const { checkStorageLimit } = await import('../usage');
          const overLimit = await checkStorageLimit(db, project, data?.length ?? 0);
          if (overLimit) return error('resource-exhausted', overLimit);
        }
        const r2Key = prefixedKey(projectId, folder, key);
      ...
      case 'storage.list': {
        ...
        const fullPrefix = folderPrefix(projectId, folder) + extraPrefix;
        const limit = rawLimit && rawLimit > 0 ? rawLimit : 100;
        const fpLen = folderPrefix(projectId, folder).length;
      ...
      case 'storage.delete_folder': {
        ...
        const prefix = folderPrefix(projectId, name);
      ...
      case 'storage.list_folders': {
        // List distinct folder prefixes under the project.
        const projectPrefix = `${projectId}/`;
        const listed = await bucket.list({ prefix: projectPrefix, delimiter: '/' });
        const folders = (listed.delimitedPrefixes ?? []).map((dp) => {
          const name = dp.endsWith('/') ? dp.slice(projectPrefix.length, -1) : dp.slice(projectPrefix.length);
          return {
            name,
            public: false,
